...
const communities = Array.from(
  new Set(metrics.filter((d) => d.metric_family === "publication").flatMap((d) => d.community || []))
).sort();
const community = view(
  Inputs.select(["All communities", ...communities], {
    label: "Community",
...
const selected = metrics.filter(
  (d) =>
    ...
    (community === "All communities" || (d.community || []).includes(community))
);
...
<div class="card">${
  resize((width) =>
    Plot.plot({
      ...
      y: { axis: null, domain: movers.map((d) => d.output_name) },
      marks: [
        Plot.barX(movers, {
          x: "yearly_delta",
          y: "output_name",
...
<div class="card">${
  resize((width) =>
    Plot.plot({
      ...
      y: { axis: null, domain: topOutputs.map((d) => d.output_name) },
      marks: [
